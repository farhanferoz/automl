...
X_reg, y_reg = make_regression(
    n_samples=100, n_features=5, noise=5.0, random_state=random_seed
)
y_reg = (
    y_reg + abs(y_reg.min()) + 1
)  # Ensure y is positive for certain percentile calcs

# Split data for initial training (for AutoML's CV) and final test evaluation
X_train_initial_reg, X_test_full_reg, y_train_initial_reg, y_test_full_reg = (
    train_test_split(X_reg, y_reg, test_size=0.2, random_state=random_seed)
)
...
automl_reg.train(
    X_train_initial_reg, y_train_initial_reg, models_to_consider=models_for_reg
)
...
if automl_reg.best_model_name:
    ...
    test_rmse = calculate_metric(
        y_test_full_reg, y_pred_test, Metric.RMSE, TaskType.REGRESSION
    )
    logger.info(f"Best model test RMSE (original scale): {test_rmse:.4f}")
    logger.info(
        f"Sample predictions (first 5, original scale): {y_pred_test[:5].round(2)}"
    )
    ...
    try:
        ...
        logger.info(
            f"Mean uncertainty estimate (original scale): {np.mean(uncertainty_values):.4f}"
        )
        logger.info(
            f"Uncertainty estimates (first 5, original scale): {uncertainty_values[:5].round(2)}"
        )
    except ValueError as e:
        logger.info(
            f"Could not get uncertainty estimates for model {automl_reg.best_model_name.value}: {e}"
        )
    except NotImplementedError as e:
        logger.info(
            f"Uncertainty prediction not implemented for {automl_reg.best_model_name.value}: {e}"
        )
    ...
    feature_importance_summary = automl_reg.get_feature_importance(
        X_test_full_reg, feature_names=feature_names_reg
    )
    if "error" not in feature_importance_summary:
        ...
        serializable_importance = {
            k: float(v) for k, v in feature_importance_summary.items()
        }
        logger.info(
            f"Top 5 Features by SHAP Importance:\n{json.dumps(dict(list(serializable_importance.items())[:5]), indent=2)}"
        )
    ...
    retrained_results_reg = automl_reg.retrain_with_selected_features(
        X_full_train=X_reg,
        y_full_train=y_reg,
        X_full_test=X_test_full_reg,
        y_full_test=y_test_full_reg,
