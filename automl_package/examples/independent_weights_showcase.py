...
def create_synthetic_data(
    n_samples: int = 1000, random_seed: int | None = None
    ...
    y_true = np.piecewise(
        ...
        [
            ...
            lambda x_nonlin: 0.5 * x_nonlin
            + np.sin(2 * np.pi * x_nonlin) * np.cos(1 * np.pi * x_nonlin)
            + np.sin(4 * np.pi * x_nonlin),
        ],
    )
...
def plot_results_bokeh(
    ...
    # Noisy Data
    noisy_data_glyph = p.scatter(
        x_data.flatten(), y_data, color="gray", alpha=0.3, size=8
    )

    # True Function
    true_function_glyph = p.line(
        x_data.flatten(), y_true, line_dash="dashed", line_color="black", line_width=2
    )
    ...
    for i, (name, y_pred) in enumerate(predictions.items()):
        ...
        legend_items.append(
            LegendItem(
                label=f"{name} (MSE: {results[name]:.4f})", renderers=[line_glyph]
            )
        )
...
def run_showcase() -> None:
    ...
    output_dir = "independent_weights_showcase_results"
    ...
    results_df = pd.DataFrame.from_dict(
        results, orient="index", columns=["MSE"]
    ).sort_values(by="MSE")
    print(results_df)
