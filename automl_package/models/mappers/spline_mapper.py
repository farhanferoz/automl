...
class SplineMapper(BaseMapper):
    ...
    def _fit(self, probas: np.ndarray, y_original: np.ndarray, **kwargs: Any) -> dict[str, Any]:  # noqa: ARG002
        ...
        self._y_min = float(np.min(y_original))
        self._y_max = float(np.max(y_original))
        ...
        # Default smoothing: scale with data variance to prevent wild extrapolation
        # in sparse probability regions (common when classifier outputs are U-shaped).
        effective_s = self.spline_s
        if effective_s is None:
            effective_s = len(probas) * np.var(y_original)

        try:
            self.model = UnivariateSpline(x=probas, y=y_original, k=effective_k, s=effective_s)
        except Exception as e:
            logger.error(f"Error fitting spline with k={effective_k}, s={effective_s}: {e}. Falling back to k=1.")
            self.model = UnivariateSpline(x=probas, y=y_original, k=1, s=effective_s)

        y_pred_train = self._clamp(self.model(probas))
        if np.isnan(y_pred_train).any():
            logger.warning("Spline predictions contain NaN values. Replacing with mean.")
    ...
    def _clamp(self, predictions: np.ndarray) -> np.ndarray:
        """Clamps predictions to the training target range with a small margin."""
        margin = 0.1 * (self._y_max - self._y_min)
        return np.clip(predictions, self._y_min - margin, self._y_max + margin)
    ...
    def _predict(self, probas_new: np.ndarray) -> np.ndarray:
        ...
        predictions = self._clamp(self.model(probas_new_clipped).flatten())
        if np.isnan(predictions).any():
            logger.warning("Spline predictions contain NaN values. Replacing with fallback value.")
