...
class BaseSelectionStrategy(ABC):
    ...
    def __init__(self, model_instance: Any) -> None:
        ...
        self.mode_selection_probs: torch.Tensor | None = (
            None  # To store probabilities for external use (e.g., classifier_logits_out)
        )
    ...
    def setup_optimizers(self, policy_params: Any) -> None:
        ...
        self.policy_optimizer = torch.optim.Adam(
            policy_params, lr=self.model.n_predictor_learning_rate
        )
    ...
    def _weighted_average_logic(
        self, x_input: torch.Tensor, mode_selection_probs: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        final_predictions_contribution = torch.zeros(
            x_input.size(0), self.model.regression_output_size
        ).to(x_input.device)
        ...
        selected_k_values = torch.where(
            selected_k_indices
            == mode_selection_probs.size(1)
            - 1,  # If it's the last mode (direct regression)
        ...
        for i in range(mode_selection_probs.size(1)):
            prob_i = mode_selection_probs[:, i].unsqueeze(
                1
            )  # Probability for this mode

            if (
                i == mode_selection_probs.size(1) - 1
            ):  # This is the direct regression mode
                predictions_for_mode = self.model.direct_regression_head(x_input)
            else:  # This is a probabilistic path with k_val = i + 2
                k_val = i + 2
                predictions_for_mode = self.model._compute_predictions_for_k(
                    x_input, k_val
                )
    ...
    def _hard_selection_logic(
        self, x_input: torch.Tensor, mode_selection_one_hot: torch.Tensor
        ...
        for i in range(mode_selection_one_hot.size(1)):
            ...
            if len(selected_indices) > 0:
                if i == mode_selection_one_hot.size(1) - 1:  # Direct regression mode
                    predictions_for_mode = self.model.direct_regression_head(
                        x_input[selected_indices]
                    )
                else:  # Probabilistic path with k_val = i + 2
                    k_val = i + 2
                    predictions_for_mode = self.model._compute_predictions_for_k(
                        classifier_raw_logits[selected_indices], k_val
                    )
