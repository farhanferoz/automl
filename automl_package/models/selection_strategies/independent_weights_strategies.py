...
class IndependentWeightsGumbelSoftmaxStrategy(BaseSelectionStrategy):
    ...
    def select_n(self, x_input: torch.Tensor, n_logits: torch.Tensor) -> SelectionOutput:
        ...
        :return: A tuple containing the actual number of layers, the probabilities for the number of layers, the logits for the number of layers, and the log probability.
        """
        if n_logits is None:
    ...
    def forward(self, x_input: torch.Tensor, n_logits: torch.Tensor) -> ForwardOutput:
        ...
        :param x_input: The input tensor.
        :param n_logits: The logits for the number of layers.
        :return: A tuple containing the input tensor, the actual number of layers, the probabilities for the number of layers, the logits for the number of layers, and the log probability.
        """
...
class IndependentWeightsNoneStrategy(BaseSelectionStrategy):
    ...
    def select_n(self, x_input: torch.Tensor, n_logits: torch.Tensor) -> SelectionOutput:
        """Selects the number of layers 'n' by always choosing the maximum number of layers.

        :param x_input: The input tensor.
        :param n_logits: The logits for the number of layers.
        :return: A tuple containing the actual number of layers, the probabilities for the number of layers, the logits for the number of layers, and the log probability.
        """
        n_actual = torch.full((x_input.shape[0],), self.model.max_hidden_layers, dtype=torch.long)
        n_probs = torch.zeros(x_input.shape[0], self.model.max_hidden_layers)
        n_probs[:, -1] = 1.0
        log_prob = torch.log(n_probs.max(dim=-1).values)
        return n_actual, n_probs, n_logits, log_prob
