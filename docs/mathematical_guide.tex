...
\date{April 2026 (rev.\ post Phase 8)}
...
in the AutoML package. All sections describe currently implemented behavior
unless explicitly flagged otherwise (e.g., Laplace-approximation BNN in
§\ref{sec:laplace}).
...
CQR & Quantile regression + conformal calibration & --- \\
\bottomrule
\end{tabular}
\end{center}
...
The Fisher information $h = \tfrac{1}{2}$ is preferred over the observed
Hessian $\tfrac{1}{2}(y-\mu)^2/\sigma^2$ because it is always positive and
gives more stable tree splits.
...
\begin{implnote}
The collapse is \emph{silent}: on discrete or heavily tied targets the effective $K$ can be
smaller than requested, with no warning emitted. Callers should check the returned boundary
count rather than assume the requested $K$.
\end{implnote}
...
\begin{implnote}
Masking shares the logits across $K$, but the percentile \emph{bin boundaries} do not nest:
they are recomputed independently for each candidate $K$ (the $K{=}2$ edges are not a subset of
the $K{=}3$ edges), so ``class $k$'' denotes a different slice of the target at each $K$. This
is harmless under likelihood-only training (\texttt{REGRESSION\_ONLY}), where boundaries never
enter the loss, but it makes the cross-entropy term unsound under dynamic $K$ — see the caveat
at Eq.~\eqref{eq:total_loss}.
\end{implnote}
...
When \texttt{optimization\_strategy = REGRESSION\_ONLY}: the cross-entropy term is disabled
entirely and the classifier is trained only through the combined predictive likelihood (the
gradient of $\mathcal{L}_{\mathrm{regression}}$ flowing back through the class probabilities).
No class-membership targets — and hence no bin boundaries — enter the loss. This is the
\emph{validated configuration for dynamic $K$}: it is the training mode of the two-stage
recipe of record (frozen likelihood-only fit, then a post-hoc distilled selector), and it is
structurally exempt from the class-identity conflict described at Eq.~\eqref{eq:total_loss}.
...
Caveat: this ablation predates the dynamic-$K$ class-identity finding (see
Eq.~\eqref{eq:total_loss}); for deployment, dynamic $K$ is validated only with likelihood-only
training (\texttt{REGRESSION\_ONLY}).
\end{implnote}
...
\begin{remark}
The weighted-sum form is essential. Overwriting entries of
$\mathbf{p}_{\mathrm{soft}}$ with hard one-hot values via indexed assignment
severs the gradient path through the gating parameters and makes the
strategy equivalent to a frozen argmax.
\end{remark}
...
\]
where $c^*$ is the true bin index and $\tilde{\ell}$ are masked logits. Under dynamic $K$,
$c^*$ is computed against the boundary set of the $K$ that sample selected on that forward
pass; since boundary sets for different $K$ are not refinements of one another, $c^*$ is not a
stable class identity across $K$ (see below).
...
\begin{implnote}
\textbf{Caveat (dynamic $K$, verified 2026-07).} With dynamic $K$ the
$\mathcal{L}_{\mathrm{CE}}$ term is structurally unsound: each sample's target $c^*$ is built
from the percentile boundaries of that sample's selected $K$, and those boundary sets do not
nest across $K$. Logit~0 is therefore supervised as ``bottom $1/K$ of $y$'' for whatever $K$
each sample selects — the same component asked to mean different things at different
resolutions, with nothing in the input distinguishing the regime. The validated configuration
for dynamic $K$ is \texttt{REGRESSION\_ONLY} (cross-entropy disabled; see \emph{Optimization
Strategies}), under which class identity is carried by the shared masked-prefix logits alone
and remains stable across $K$. Cross-entropy-active dynamic $K$ should be treated as
unsupported pending a $K$-stable class semantics (e.g.\ boundaries anchored to a single
$K_{\max}$ grid, or refinement binning).
\end{implnote}
...
%=============================================================================
\section{Conformalized Quantile Regression (Romano et al., 2019)}
\label{sec:cqr}
%=============================================================================

CQR calibrates the intervals produced by a quantile regressor (§18) via
split conformal prediction (§\ref{sec:conformal}) so that coverage is
distribution-free and finite-sample-valid.

Fit $(\hat{q}_{\alpha/2}, \hat{q}_{1-\alpha/2})$ on the training fold.
On a held-out calibration set $\{(\bx_i, y_i)\}_{i=1}^{n}$ compute the
conformity scores
\begin{equation}
  R_i = \max\!\left(\hat{q}_{\alpha/2}(\bx_i) - y_i,\; y_i - \hat{q}_{1-\alpha/2}(\bx_i)\right),
  \label{eq:cqr_score}
\end{equation}
which measure how far each target falls outside the predicted interval
(negative when inside). Take the finite-sample-corrected quantile
\[
  \hat{Q}_\alpha = \text{Quantile}\!\left(\{R_i\}, \; \frac{\lceil (n+1)(1-\alpha) \rceil}{n}\right).
\]

\textbf{Calibrated interval:}
\[
  C(\bx) = \left[\hat{q}_{\alpha/2}(\bx) - \hat{Q}_\alpha,\; \hat{q}_{1-\alpha/2}(\bx) + \hat{Q}_\alpha\right].
\]

Coverage $\Pr(y \in C(\bx)) \geq 1 - \alpha$ holds under exchangeability.
Unlike marginal split conformal (§\ref{sec:conformal}), the CQR interval
width varies with $\bx$ because the base quantile regressor is
heteroscedastic.
...
\emph{Not yet implemented --- included for comparison completeness.}
...
Distribution-free intervals that wrap \emph{any} fitted model. Given a
calibration set $\{(\bx_i, y_i)\}_{i=1}^{n}$ and miscoverage $\alpha$.

\subsection{Split Conformal (Marginal)}
\label{sec:conformal_marginal}
...
Interval width is a constant $2\hat{q}_\alpha$ --- over-covers in
low-noise regions and under-covers in high-noise regions on a
per-point basis.

\subsection{Locally-Adaptive Conformal (Lei \& Wasserman, 2014)}
\label{sec:conformal_adaptive}

Given a base model that supplies both a point prediction $\hat{f}(\bx)$
and an uncertainty estimate $\hat{\sigma}(\bx)$, normalize the
nonconformity scores by the predicted standard deviation:
\begin{equation}
  \tilde{R}_i = \frac{|y_i - \hat{f}(\bx_i)|}{\hat{\sigma}(\bx_i)},
  \qquad
  \hat{Q}_\alpha = \text{Quantile}\left(\{\tilde{R}_i\},\; \frac{\lceil (n+1)(1-\alpha) \rceil}{n}\right).
  \label{eq:lacw_quantile}
\end{equation}

\textbf{Adaptive interval:}
\[
  C(\bx) = \left[\hat{f}(\bx) - \hat{Q}_\alpha \cdot \hat{\sigma}(\bx),\;
                  \hat{f}(\bx) + \hat{Q}_\alpha \cdot \hat{\sigma}(\bx)\right].
\]

The same coverage guarantee $\Pr(y \in C(\bx)) \geq 1 - \alpha$ holds, but
intervals now contract where the base model is confident and widen where
it is uncertain. This gives \emph{conditional} coverage if
$\hat{\sigma}(\bx)$ tracks the true residual scale.
...
\[
  \text{NLL} = \frac{1}{N}\sum_{i} \frac{1}{2}\left(\log(2\pi) + \log\hat{\sigma}_i^2 + \frac{(y_i - \hat{y}_i)^2}{\hat{\sigma}_i^2}\right).
\]

\textbf{CRPS.} For $\mathcal{N}(\mu, \sigma^2)$:
\begin{equation}
  \text{CRPS} = \sigma \left[
  ...
  \label{eq:crps}
\end{equation}
Strictly proper. Generalizes to non-Gaussian predictive distributions via
numerical integration.

\textbf{Pinball (quantile) loss.}
$L_\tau(y, \hat{q}_\tau) = \max(\tau(y - \hat{q}_\tau), (1-\tau)(\hat{q}_\tau - y))$;
at $\tau = 0.5$ equals MAE.

\subsection{Interval Metrics}
...
\textbf{Winkler score.} For interval $[L, U]$ at level $(1-\alpha)$:
\begin{equation}
  W_\alpha = (U - L) +
  ...
  \label{eq:winkler}
\end{equation}
Strictly proper scoring rule for intervals.

\subsection{Calibration via PIT (Kuleshov et al., 2018)}
\label{sec:pit_cal}

Probability Integral Transform: $U_i = F(y_i \mid \bx_i)$. If the
predictive distribution is calibrated, $U \sim \text{Uniform}(0, 1)$.

\textbf{Calibration curve:} for target quantiles
$p_0 < \cdots < p_B \in [0,1]$,
$\hat{p}_j = \frac{1}{N}\sum_i \mathbf{1}[U_i \leq p_j]$.

\textbf{Regression ECE:} $\text{ECE} = \frac{1}{B+1}\sum_{j} |p_j - \hat{p}_j|$.

\textbf{Miscalibration area:} $\int_0^1 |\hat{p}(t) - t| \, dt$ (trapezoidal).
More robust than bin-averaged ECE.

\textbf{Sharpness:} $\frac{1}{N}\sum_{i} \hat{\sigma}_i$. Always reported
jointly with calibration --- a calibrated model with wide intervals is
useless.

\subsection{Post-hoc Recalibration}
\label{sec:isotonic}

Isotonic regression from predicted CDF values to empirical CDF on a
held-out set (Kuleshov et al., 2018). Let $\{U_i^{\mathrm{cal}}\}$ be PIT
values on the calibration set. Fit a monotone $\hat{R}: [0,1] \to [0,1]$
with
\[
  \hat{R}(U_{(i)}^{\mathrm{cal}}) \approx i / n_{\mathrm{cal}}.
\]
At test time apply $U_i^{\mathrm{recal}} = \hat{R}(U_i)$. Report metrics
as ``+recal'' variants to separate raw calibration quality from
post-hoc-fixable calibration quality.

\subsection{PIT Uniformity Tests}

Sharper than visual inspection of histograms. Let $\{U_i\}$ be the PIT
values.

\textbf{Kolmogorov-Smirnov:} $\text{KS} = \sup_t |\hat{F}_N(t) - t|$
where $\hat{F}_N$ is the empirical CDF of $\{U_i\}$.

\textbf{Cramér-von Mises:}
$\text{CvM} = \tfrac{1}{12N} + \sum_{i=1}^{N}\!\left(\tfrac{2i-1}{2N} - U_{(i)}\right)^2$.
More sensitive to global deviations.

\textbf{Anderson-Darling:} weights deviations by $1/[t(1-t)]$, emphasizing
the tails of the distribution. Implemented by transforming
$Z_i = \Phi^{-1}(U_i)$ and applying the standard normal AD test, since
SciPy does not expose AD directly for the uniform distribution.
...
\textbf{PIT uniformity:} KS, CvM, and AD tests on
$\{F(z_{\mathrm{spec},i}|\bx_i)\}$ vs $\mathrm{Uniform}(0,1)$
(§\ref{sec:pit_cal}). CvM and AD are more sensitive to tail deviations,
which drive outlier rates.

\textbf{CDE loss (Izbicki \& Lee, 2017):}
\begin{equation}
  \mathrm{CDE\_loss} = -\frac{1}{N}\sum_i \log \hat{p}(z_{\mathrm{spec},i} \mid \bx_i),
  \label{eq:cde}
\end{equation}
the negative log-likelihood of the true redshift under the model's
predicted conditional density. Standard metric for full-PDF quality
