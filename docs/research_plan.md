...
**L2.1 — UCI Airfoil Self-Noise (NASA).** 1503 samples, 5 features (corrected 2026-07-20: the UCI page
lists 5, and the enumeration below has always contained 5 — the "6" was an error) (frequency in Hz, angle of attack in degrees, chord length in m, free-stream velocity in m/s, suction-side displacement thickness in m), target: scaled sound pressure level (dB). Physics: self-induced airfoil noise from boundary-layer turbulence, measured in an anechoic wind tunnel on NACA 0012 airfoils. Heteroscedastic in the high-frequency regime. Standard tabular regression benchmark since 1989. No missing values. Available at https://archive.ics.uci.edu/dataset/291/airfoil+self+noise.
